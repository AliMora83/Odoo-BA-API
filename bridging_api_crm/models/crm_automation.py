...
class CrmLeadAutomation(models.Model):
    ...
    def _cron_process_paid_leads(self):
        ...
        _logger.info("Found %d paid leads to process", len(paid_leads))
        ...
        for lead in paid_leads:
            try:
                ...
                success_count += 1
                _logger.info("Processed lead: %s (ID: %d)", lead.name, lead.id)
            except Exception as e:
                _logger.error("Failed lead %s (ID: %d): %s", lead.name, lead.id, str(e))
                continue

        _logger.info("=== Completed: %d/%d leads processed ===", success_count, len(paid_leads))
        return True

    def _process_paid_lead_workflow(self):
        """Complete workflow: Lead -> Opportunity -> Sales Order -> Invoice + Payment"""
        self.ensure_one()
        _logger.info("Processing workflow for: %s", self.name)
        ...
        _logger.info("Sales Order %s confirmed", sale_order.name)
        ...
        # Step 5: Post Invoice
        invoice.action_post()
        _logger.info("Invoice %s posted", invoice.name)
        ...
        self.message_post(
            body=_(
                ...
                'so': sale_order.name,
                'inv': invoice.name,
                'amt': '%s%.2f' % (invoice.currency_id.symbol, invoice.amount_total),
            },
    ...
    def _register_payment_for_invoice(self, invoice):
        """Register a payment for the lead fee invoice."""
        self.ensure_one()
    ...
    def _create_sale_order_from_opportunity(self):
        ...
        order_vals = {
            ...
            'origin': 'Opportunity: %s' % self.name,
            'note': 'Auto-created lead fee from Bridging Africa. Quote ID: %s' % (self.quote_id or 'N/A'),
            'order_line': [(0, 0, {
                'product_id': product.id,
                'name': 'Lead Fee - %s - Job #%s' % (self.service_id.name if self.service_id else 'Service', self.quote_id or ''),
                'product_uom_qty': 1,
                'price_unit': 1.0,
                'tax_id': [(6, 0, [])],
            })],
